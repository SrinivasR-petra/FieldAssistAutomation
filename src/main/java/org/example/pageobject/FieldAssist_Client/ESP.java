...
public class ESP {
    ...
    public ESP(WindowsDriver<WindowsElement> _FASession) {
    ...
    private By ESP_Menu = By.name("ESP");
    private By ESPWells_Menu = By.name("ESP Wells");
    private By ESP_AllWellsTab = By.name("ESPWellsHSpltWells");
    private By ESP_AllWells_WellsGrid = MobileBy.AccessibilityId("ESPWellsHSpltWells");
    private By Grid_DataTable = MobileBy.AccessibilityId("tableControl1");
    private By ESPWellsSubDetailsTab2 = MobileBy.AccessibilityId("ESPWellsSubDetailsTab2");

    public void Navigate_to_ESPWells_Screen() {
        FASession.findElement(ESP_Menu).click();
        FASession.findElement(ESPWells_Menu).click();
    }

    public void Navigate_to_ESP_AllWells_Screen_and_Select_a_Well(String DwellName) {
        Navigate_to_ESPWells_Screen();
        FASession.findElement(ESP_AllWellsTab).isDisplayed();
        FASession.findElement(ESP_AllWellsTab).click();
        var espAllWells = FASession.findElement(ESP_AllWells_WellsGrid);
        var wellDataGrid = espAllWells.findElement(Grid_DataTable);
    ...
    public void Verify_WellNote_is_added_or_not(String Comment){
        MobileElement espAllWells = FASession.findElement(ESP_AllWells_WellsGrid);
        MobileElement wellDataGrid = espAllWells.findElement(Grid_DataTable);
        String appWellNote;
        while (true) {
        ...
        while (true) {
            List<MobileElement> tabItems = FASession.findElement(ESPWellsSubDetailsTab2).findElementsByXPath("//TabItem");
            for (MobileElement tabItem : tabItems) {
                ...
                if(tabName.equals("ALSExcLowisNotesGrid")) {
                    ...
                    MobileElement notesDataGrid = FASession.findElement(ESPWellsSubDetailsTab2).findElement(Grid_DataTable);
...
//                    FASession.findElement(ESPWellsSubDetailsTab2).findElement(Grid_DataTable).click();
//                    actions.keyDown(Keys.CONTROL).sendKeys(Keys.ARROW_UP).sendKeys(Keys.ARROW_LEFT).keyUp(Keys.CONTROL).sendKeys(Keys.TAB).build().perform();
                    List<MobileElement> notesComment = FASession.findElement(ESPWellsSubDetailsTab2).findElement(Grid_DataTable).findElementsByXPath("//*");
    ...
    private By ExceptionsTab = By.name("ESPWellsMainExcTabs");
    private By Exc_CurrentTab = By.name("ESPWellsExcCurrHSplt");
    private By Exc_CurrentGrid = MobileBy.AccessibilityId("ESPWellsExcCurrHSplt");
    private By Exc_DeferredTab = By.name("ESPWellsExcDefHSpltTab");
    private By Exc_DeferredGrid = MobileBy.AccessibilityId("ESPWellsExcDefHSpltTab");



    public void Navigate_to_ESP_ExcCurrentGrid_and_Select_a_Well(String DWellName) {
