...
public class GasLift {
    ...
    public GasLift(WindowsDriver<WindowsElement> _FASession) {
    ...
    private By GLWells_Menu = By.name("Gas Lift Wells");
    private By GL_AllWellsTab = By.name("GLWellsHSpltWells");
    private By GL_AllWells_WellsGrid = MobileBy.AccessibilityId("GLWellsHSpltWells");
    private By Grid_DataTable = MobileBy.AccessibilityId("tableControl1");
    private By GLWellsSubDetailsTab2 = MobileBy.AccessibilityId("AlarmsTabs");

    public void Navigate_to_GasLiftWells_Screen(){
        FASession.findElement(GL_Menu).click();
        FASession.findElement(GLWells_Menu).click();
    }

    public void Navigate_to_GasLift_AllWells_Screen_and_Select_a_Well(String DwellName) {
        Navigate_to_GasLiftWells_Screen();
        var glAllWells = FASession.findElement(GL_AllWellsTab);
        glAllWells.isDisplayed();
        glAllWells.click();
        var gasliftAllWells = FASession.findElement(GL_AllWells_WellsGrid);
        var wellDataGrid = gasliftAllWells.findElement(Grid_DataTable);
    ...
    public void Verify_WellNote_is_added_or_not(String Comment){
        MobileElement gasliftAllWells = FASession.findElement(GL_AllWells_WellsGrid);
        MobileElement wellDataGrid = gasliftAllWells.findElement(Grid_DataTable);
        String appWellNote;
        while (true) {
        ...
        while (true) {
            List<MobileElement> tabItems = FASession.findElement(GLWellsSubDetailsTab2).findElementsByXPath("//TabItem");
            for (MobileElement tabItem : tabItems) {
                ...
                if(tabName.equals("ALSExcLowisNotesGrid")) {
                    ...
                    MobileElement notesDataGrid = FASession.findElement(GLWellsSubDetailsTab2).findElement(Grid_DataTable);
...
//                    FASession.findElement(GLWellsSubDetailsTab2).findElement(Grid_DataTable).click();
//                    actions.keyDown(Keys.CONTROL).sendKeys(Keys.ARROW_UP).sendKeys(Keys.ARROW_LEFT).keyUp(Keys.CONTROL).sendKeys(Keys.TAB).build().perform();
                    List<MobileElement> notesComment = FASession.findElement(GLWellsSubDetailsTab2).findElement(Grid_DataTable).findElementsByXPath("//*");
    ...
    public void Navigate_to_GasLift_ExcCurrentGrid_and_Select_a_Well(String DWellName) {
